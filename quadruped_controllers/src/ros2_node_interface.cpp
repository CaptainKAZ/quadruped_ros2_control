...
void OdomTfPublisher::init(rclcpp::Node::SharedPtr &&node,
  ...
  Ros2NodeInterfaceBase::init(std::forward<decltype(node)>(node), state, command);
  ...
  tf_message.transforms.front().header.frame_id = "odom";
  tf_message.transforms.front().child_frame_id = "world";
...
void OdomTfPublisher::update(const rclcpp::Time &current_time) {
  ...
  if (rt_odom_tf_pub_->trylock()) {
    ...
    rt_odom_tf_pub_->msg_.transforms.push_back(transform);
...
void GroundTruthSubscriber::init(rclcpp::Node::SharedPtr &&node,
  ...
  cmd_sub_ = node->create_subscription<nav_msgs::msg::Odometry>(
      "/ground_truth/pose", rclcpp::SystemDefaultsQoS(),
