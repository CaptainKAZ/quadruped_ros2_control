...
QuadrupedSwingStanceController::init(const std::string &controller_name) {
  ...
  auto swing_stance_param_callback = [this](std::vector<rclcpp::Parameter> params) {
    auto result = rcl_interfaces::msg::SetParametersResult();
    for (auto &param : params) {
      auto update_mat3 = [&result, &param](std::string name,
                                           Mat3<double> &mat) {
      ...
      update_mat3("kp_stance", kp_stance_);
      update_mat3("kd_stance", kd_stance_);
      update_mat3("kp_swing", kp_swing_);
      update_mat3("kd_swing", kd_swing_);
    }
    return result;
  };
...
controller_interface::return_type QuadrupedSwingStanceController::update() {
  ...
  for (size_t i = 0; i < 4; i++) {
    if ((swing_stance_config_->swing_duration_ > 1e-3 &&
         (now - swing_stance_config_->cmd_time_).seconds() <
             swing_stance_config_->swing_duration_) ||
        (now - swing_stance_config_->cmd_time_).seconds() < 1e-1) {
...
void QuadrupedSwingStanceController::setSwing(size_t leg, Eigen::Vector3d &final_pos, double height, double duration){
...
void QuadrupedSwingStanceController::setStance(size_t leg, Eigen::Vector3d &force_ff){
  rcpputils::check_true(leg < 4, "leg index out of range");
