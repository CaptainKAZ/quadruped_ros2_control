...
public:
  ...
  void buildStateSpace(double mass, const Matrix3d& inertia, const QuadrupedState& state);
  void buildQp(double dt);

  const Matrix<double, Dynamic, Dynamic, Eigen::RowMajor>& buildHessianMat();
  const VectorXd& buildGVec(double gravity, const QuadrupedState& state, const Matrix<double, Dynamic, 1>& traj);
